...
- Self-Projects (publicly shipped, recruiter-verifiable):
  ...
  - Go Listings API (link: github.com/davidalexander24/go-listings-api): a marketplace REST API written in Go using only the standard library net/http (no framework), with PostgreSQL via pgx, Redis cache-aside, keyset pagination, a bounded goroutine worker pool, graceful shutdown, Docker, and integration tests against a dockerized Postgres. Note: David picked up Go recently (2026) and is ramping fast; present this as a strong first Go project, not deep Go expertise.
...
- Tech Stack: TypeScript, JavaScript, React 19, Next.js 16, Node.js, NestJS, TailwindCSS; Java with Spring Boot (plus LibGDX for game development); Go (recent); PostgreSQL with both Drizzle ORM and Prisma; Redis; Zod, Jest, JUnit, NextAuth; Docker and self-hosted deployments; and multi-provider LLM orchestration (Gemini and OpenAI-compatible APIs).
