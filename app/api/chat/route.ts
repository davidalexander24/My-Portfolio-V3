...
- Current Roles:
  ...
  - Software Engineer at Exercise FTUI (2024-present): Lead Developer for the Teknik Charity Run platform; built end-to-end event and registration platforms handling 2000+ users and 100+ successful transactions via Midtrans.
- Self-Projects (publicly shipped, recruiter-verifiable):
  - Prime Capital Ledger (live, primecapitaledger.site): a full-stack portfolio-management and financial-analytics platform with an immutable Decimal(19,4) transaction ledger, multi-brokerage PDF statement ingestion, and analytics like Sharpe ratio and max drawdown. Built with Next.js 16, Prisma, PostgreSQL (Neon), Redis (Upstash), and NextAuth.
  - AI Workflow Automation Engine (live): a reusable AI workflow-template dashboard that routes 15 models across 5 providers (Google Gemini, OpenAI, Groq, Cerebras, OpenRouter) with per-run model and temperature control and server-only secret handling. Built with Next.js 16, NestJS 11, and Prisma; containerized with Docker and self-hosted.
